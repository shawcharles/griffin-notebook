...
def plugin_no_server(mocker, qtbot):
    ...
    mocker.patch('griffin_notebook.widgets.main_widget.ServerManager',
...
def test_view_server_info(mocker, plugin_no_server):
    ...
    mock_ServerInfoDialog = mocker.patch(
        'griffin_notebook.widgets.main_widget.ServerInfoDialog')
...
@pytest.mark.parametrize('config_value', ['same as griffin', 'dark', 'light'])
@pytest.mark.parametrize('griffin_is_dark', [True, False])
def test_dark_theme(mocker, plugin_no_server, config_value, griffin_is_dark):
    plugin_no_server.set_conf('theme', config_value)
    mocker.patch('griffin_notebook.widgets.main_widget.is_dark_interface',
                 return_value=griffin_is_dark)

    value = plugin_no_server.get_widget().dark_theme

    expected = (config_value == 'dark' or
                (config_value == 'same as griffin' and griffin_is_dark))
