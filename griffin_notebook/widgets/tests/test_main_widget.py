...
# Local imports
from griffin_notebook.tests.test_plugin import (
    is_kernel_up, prompt_present, text_present)
from griffin_notebook.widgets.main_widget import NotebookMainWidget
...
def test_save_notebook_as_with_error(main_widget, mocker, qtbot, tmpdir):
    ...
    # Set up mocks
    name = osp.join(str(tmpdir), 'save.ipynb')
    mocker.patch('griffin_notebook.widgets.notebooktabwidget.getsavefilename',
                 return_value=(name, 'ignored'))
    mocker.patch('griffin_notebook.widgets.notebooktabwidget.nbformat.write',
                 side_effect=PermissionError)
    mock_critical = mocker.patch('griffin_notebook.widgets.notebooktabwidget'
...
def test_open_console_when_no_kernel(main_widget, qtbot, mocker):
    ...
    MockMessageBox = mocker.patch(
        'griffin_notebook.widgets.main_widget.QMessageBox')
...
def test_close_nonexisting_notebook(main_widget, qtbot, tmpdir):
    ...
    Regression test for griffin-ide/griffin-notebook#187."""
