...
from griffin.utils.programs import get_temp_dir
from griffin.widgets.tabs import Tabs

# Local imports
from griffin_notebook.utils.localization import _
from griffin_notebook.widgets.client import NotebookClient
...
class NotebookTabWidget(Tabs, GriffinConfigurationAccessor):
    """
    ...
    def create_new_client(self, filename=None):
        ...
        if not filename:
            ...
            kernelspec = dict(display_name='Python 3 (Griffin)',
                              name='python3')
    ...
    def get_interpreter(self):
        ...
        This function looks in the Griffin configuration to determine and
    ...
    def handle_server_timed_out_or_error(self, process):
        ...
        QMessageBox.critical(
            self,
            _("Server error"),
            _("The Griffin Notebook server failed to start or it is "
              "taking too much time to do it. Please select 'Server info' "
              "in the plugin's option menu to check for errors."))
        # Create a welcome widget, see gh:griffin-ide/griffin-notebook#93
        self.untitled_num -= 1
        self.maybe_create_welcome_client()
        return None
