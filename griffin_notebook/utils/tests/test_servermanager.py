...
# Local imports
from griffin_notebook.utils.servermanager import (
...
def test_start_server(mocker, dark, under_home):
    ...
    mock_QProcess = mocker.patch(
        'griffin_notebook.utils.servermanager.QProcess', spec=QProcess)
    mocker.patch(
        'griffin_notebook.utils.servermanager.get_home_dir', return_value='foo')
    if under_home:
...
def test_check_server_started_if_started(mocker, qtbot):
    ...
    fake_open = mocker.patch('griffin_notebook.utils.servermanager.open',
                             mocker.mock_open(read_data='{"foo": 42}'))
    mocker.patch('griffin_notebook.utils.servermanager.jupyter_runtime_dir',
...
def test_check_server_started_if_not_started(mocker, qtbot):
    ...
    mocker.patch('griffin_notebook.utils.servermanager.jupyter_runtime_dir',
                 return_value='runtimedir')
    mock_QTimer = mocker.patch('griffin_notebook.utils.servermanager.QTimer',
                               spec=QTimer)
    mock_process = mocker.Mock(spec=QProcess)
    server_process = ServerProcess(
        mock_process, 'notebookdir', 'interpreter', 'info.json')
...
def test_shutdown_all_servers(mocker):
    ...
    mock_shutdown = mocker.patch(
        'griffin_notebook.utils.servermanager.serverapp.shutdown_server')
